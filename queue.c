...
bool q_insert_head(struct list_head *head, char *s)
{
    ...
    temp->value = malloc(len * sizeof(char));
    if (!(temp->value)) {
        free(temp);
        return false;
    }
    memcpy(temp->value, s, len);
    *(temp->value + len - 1) = '\0';
...
element_t *q_remove_head(struct list_head *head, char *sp, size_t bufsize)
{
    ...
    if (sp) {
        size_t len =
            (bufsize - 1) > strlen(e->value) ? strlen(e->value) : (bufsize - 1);
        /*memcpy element value to sp and assign a NULL terminator to the end of
         * the MEM. space*/
